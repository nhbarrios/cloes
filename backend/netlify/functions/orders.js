...
//
// PUT    /api/orders?id=8&action=cancel
//   -> marca el pedido como cancelado. Deja de contar en "por cobrar",
//      en "total pedidos" y en el reporte de utilidades — pero el registro
//      no se borra, se conserva para el historial.
//
// PUT    /api/orders?id=8&action=reactivate
//   -> deshace la cancelación (vuelve a 'pendiente' o 'saldado' según lo
//      ya abonado).
//
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'PUT' && (action === 'cancel' || action === 'reactivate')) {
    ...
    try {
      ...
      if (action === 'cancel') {
        await pool.query(`UPDATE orders SET status = 'cancelado' WHERE id = ?`, [id]);
        return json(200, { orderId: Number(id), status: 'cancelado' });
      } else {
        const [[paidRow]] = await pool.query(
          'SELECT COALESCE(SUM(amount),0) AS paid FROM payments WHERE order_id = ?', [id]
        );
        const newStatus = Number(paidRow.paid) >= Number(order.total) - 0.009 ? 'saldado' : 'pendiente';
        await pool.query('UPDATE orders SET status = ? WHERE id = ?', [newStatus, id]);
        return json(200, { orderId: Number(id), status: newStatus });
      }
    } catch (err) {
      console.error(err);
  ...
  if (event.httpMethod === 'PUT') {
    ...
    try {
      ...
      for (const it of items) {
        await conn.query(
          `INSERT INTO order_items (order_id, description, quantity, unit_price, unit_cost)
           VALUES (?, ?, ?, ?, ?)`,
          [id, it.description, it.quantity, it.unitPrice, it.unitCost || 0]
        );
      }
  ...
  try {
    ...
    for (const it of items) {
      await conn.query(
        `INSERT INTO order_items (order_id, description, quantity, unit_price, unit_cost)
         VALUES (?, ?, ?, ?, ?)`,
        [orderId, it.description, it.quantity, it.unitPrice, it.unitCost || 0]
      );
    }
